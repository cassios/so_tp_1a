...
/* MARK NAME Cassios Kley Martins Marques */
...
struct andcmd {
  int type;                 // a 
  struct cmd *headcmd;   // o comando a rodar (ex.: um execcmd)
  char tailbuf[100];        // resto do buffer
};

struct subshellcmd {
  int type;                 // a 
  char buf[100];        // resto do buffer
};
...
runcmd(struct cmd *cmd)
{
  ...
  struct andcmd *andcmd;
  struct subshellcmd *subcmd;
  ...
  case ' ':
     ...
     * comandos simples. */
    char binPath[50] = "/bin/";
    if(execv(strcat(binPath, ecmd->argv[0]), ecmd->argv) == -1) {
        char usrBinPath[50] = "/usr/bin/";
        execv(strcat(usrBinPath, ecmd->argv[0]), ecmd->argv);
    }
  ...
  case 'a':
    andcmd = (struct andcmd*)cmd;
    if(fork1() == 0) {
        //filho executa head cmd
        runcmd(andcmd->headcmd);
    } else {
        wait(&r);
        //executou head cmd com sucesso
        if(r == 0) {
            runcmd(parsecmd(andcmd->tailbuf));
        } else {
            exit(1);
        }
    }
  ...
  case '(':
    subcmd = (struct subshellcmd*)cmd;
    if(fork1() == 0) {
        //filho executa sub shell
        runcmd(parsecmd(subcmd->buf));
    } else {
        wait(&r);
    }
  }    
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    /* O if verifica se foi possivel mudar de diretorio, ou seja,
...
struct cmd*
andcmd(struct cmd *headcmd, char *tail)
{
  struct andcmd *cmd;
  ...
  cmd->type = 'a';
  cmd->headcmd = headcmd;
  strcpy(cmd->tailbuf, tail);
  return (struct cmd*)cmd;
}

struct cmd*
subshellcmd(char *buf)
{
...
struct cmd *parsesubshell(char**, char*);
...
{
  struct cmd *cmd;
  cmd = parsesubshell(ps, es);
  return cmd;
}

struct cmd*
parsesubshell(char **ps, char *es)
{
  struct cmd *cmd;
